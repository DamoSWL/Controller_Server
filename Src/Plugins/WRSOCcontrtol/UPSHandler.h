...
class UPSHandler/* : public ISerialPortCalBack*/
{
public:
	UPSHandler();
	~UPSHandler();
...
public:
	std::map<INT,std::string> m_UPSID;
	std::map<std::string,std::string> m_UPSOID;
	std::map<std::string,INT> m_UPSData;
	std::map<std::string,FLOAT> m_UPSCoefficient;
