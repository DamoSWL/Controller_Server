...
UPSHandlerSNMP::UPSHandlerSNMP():m_currentID(0)
{
	
}

UPSHandlerSNMP::~UPSHandlerSNMP()
{
}



void UPSHandlerSNMP::getUPSInfo()
{
...
int UPSHandlerSNMP::cbSQLGetID(void *pIn/*传入的参数*/, int argc, char **argv, char **azColName)
{
	UPSHandlerSNMP *pT = static_cast<UPSHandlerSNMP*>(pIn);
	if(pT)
	{
...
int UPSHandlerSNMP::cbSQLGetName(void *pIn/*传入的参数*/, int argc, char **argv, char **azColName)
{
	UPSHandlerSNMP *pT = static_cast<UPSHandlerSNMP*>(pIn);
	if(pT)
	{
...
int UPSHandlerSNMP::cbSQLGetOID(void *pIn/*传入的参数*/, int argc, char **argv, char **azColName)
{
	UPSHandlerSNMP *pT = static_cast<UPSHandlerSNMP*>(pIn);
	if(pT)
	{
...
int UPSHandlerSNMP::cbSQLGetCoefficient(void *pIn/*传入的参数*/, int argc, char **argv, char **azColName)
{
	UPSHandlerSNMP *pT = static_cast<UPSHandlerSNMP*>(pIn);
	if(pT)
	{
...
BOOL UPSHandlerSNMP::queryUPSData()
{
	BOOL result = FALSE;
	for(auto id = m_UPSOID.cbegin();id != m_UPSOID.cend(); id++)
	{
		...
		char cmdLine[] = {"ups.bat"};  //将命令写入批处理文件中，通过调用进程采集数据
		...
		if(wxFileName::GetSize(wxT("ups.txt")) <= 0)
		{
			return FALSE;
		}
		...
		if(file.is_open())
		{
			while(!file.eof())
			{
				...
				if(content.find("Value") != std::string::npos)  //打开生成的ups参数文件，并解析数据
				{
...
bool UPSHandlerSNMP::writeToDataBase()
{
	auto pDataBase = GetPlug()->getDatabase();
	if (pDataBase)
	{
		...
		for(auto id = m_UPSID.cbegin(); id != m_UPSID.cend(); id++)
		{
			if(m_UPSCoefficient[id->second] > 1.0)
			{
...
DWORD WINAPI UPSHandlerSNMP::exec(LPVOID para)
{
	
	UPSHandlerSNMP* self = static_cast<UPSHandlerSNMP*>(para);
	if(self)
	{
		self->getUPSInfo();
		while (!GetPlug()->GetExit())  //每隔一段时间查询一次
		{
...
//BOOL UPSHandlerSNMP::StopCom()
//{
//	return m_serialPort.StopMonitoring();
//}
//
//void UPSHandlerSNMP::sendUPSCmd()
//{
...
//void UPSHandlerSNMP::CallBack(UINT msg, WPARAM chi, LPARAM port)
//{
//	
//	static int UPSCnt = 0;
